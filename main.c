...
void timer0_isr () interrupt 1
{
	...
	if(g.servo0_power == 1 && g.servo0_mode == 0x3 && i0++ == g.servo0_tosc ) 
	{
		...
		j0++;
 	    if(g.servo0_b >= g.servo0_a)
			set_dt0(g.servo0_a + (g.servo0_b - g.servo0_a)/2 + (char)(((int)(g.servo0_b - g.servo0_a)*(int)(sine[j0]))>>8));
		else
			set_dt0(g.servo0_b + (g.servo0_a - g.servo0_b)/2 + (char)(((int)(g.servo0_a - g.servo0_b)*(int)(sine[j0]))>>8));
	...
	if(g.servo1_power == 1 && g.servo1_mode == 0x3 && i1++ == g.servo1_tosc ) 
	{
		...
		if(g.servo1_b >= g.servo1_a)
			set_dt1(g.servo1_a + (g.servo1_b - g.servo1_a)/2 + (char)(((int)(g.servo1_b - g.servo1_a)*(int)(sine[j1]))>>8));
		else
			set_dt1(g.servo1_b + (g.servo1_a - g.servo1_b)/2 + (char)(((int)(g.servo1_a - g.servo1_b)*(int)(sine[j1]))>>8));
	}
	...
	if(g.servo2_power == 1 && g.servo2_mode == 0x3 && i2++ == g.servo2_tosc ) 
	{
		...
		if(g.servo2_b >= g.servo2_a)
			set_dt2(g.servo2_a + (g.servo2_b - g.servo2_a)/2 + (char)(((int)(g.servo2_b - g.servo2_a)*(int)(sine[j2]))>>8));
		else
			set_dt2(g.servo2_b + (g.servo2_a - g.servo2_b)/2 + (char)(((int)(g.servo2_a - g.servo2_b)*(int)(sine[j2]))>>8));
	}
    TL0       = 0x64;
    TH0       = 0xFF;
	TF0 = 0; 
...
void i2c_callback(uint8 reg, uint8 *buffer)
{
	switch(reg)
	{
		...
		case SCSO_REG_SERVO0_A: g.servo0_a = buffer[0]; break;
		case SCSO_REG_SERVO0_B: g.servo0_b = buffer[0]; break;
		case SCSO_REG_SERVO0_C: g.servo0_c = buffer[0]; break;

		case SCSO_REG_SERVO1_A: g.servo1_a = buffer[0]; break;
		case SCSO_REG_SERVO1_B: g.servo1_b = buffer[0]; break;
		case SCSO_REG_SERVO1_C: g.servo1_c = buffer[0]; break;

		case SCSO_REG_SERVO2_A: g.servo2_a = buffer[0]; break;
		case SCSO_REG_SERVO2_B: g.servo2_b = buffer[0]; break;
		case SCSO_REG_SERVO2_C: g.servo2_c = buffer[0]; break;

		case SCSO_REG_SERVO0_GOTO_A: set_dt0(g.servo0_a); g.servo0_mode = 0x0; break;
		case SCSO_REG_SERVO0_GOTO_B: set_dt0(g.servo0_b); g.servo0_mode = 0x1; break;
		case SCSO_REG_SERVO0_GOTO_C: set_dt0(g.servo0_c); g.servo0_mode = 0x2; break;

		case SCSO_REG_SERVO1_GOTO_A: set_dt1(g.servo1_a); g.servo1_mode = 0x0; break;
		case SCSO_REG_SERVO1_GOTO_B: set_dt1(g.servo1_b); g.servo1_mode = 0x1; break;
		case SCSO_REG_SERVO1_GOTO_C: set_dt1(g.servo1_c); g.servo1_mode = 0x2; break;

		case SCSO_REG_SERVO2_GOTO_A: set_dt2(g.servo2_a); g.servo2_mode = 0x0; break;
		case SCSO_REG_SERVO2_GOTO_B: set_dt2(g.servo2_b); g.servo2_mode = 0x1; break;
		case SCSO_REG_SERVO2_GOTO_C: set_dt2(g.servo2_c); g.servo2_mode = 0x2; break;

		case SCSO_REG_SERVO0_OSC: g.servo0_mode = 0x3; break;
		case SCSO_REG_SERVO1_OSC: g.servo1_mode = 0x3; break;
		case SCSO_REG_SERVO2_OSC: g.servo2_mode = 0x3; break;

		case SCSO_REG_SERVO0_TOSC: g.servo0_tosc = buffer[0]; break;
		case SCSO_REG_SERVO1_TOSC: g.servo1_tosc = buffer[0]; break;
		case SCSO_REG_SERVO2_TOSC: g.servo2_tosc = buffer[0]; break;
	}
}
