...
void i2c_init(uint8 address, void *ptr, uint8 size) {
  ...
  ve_start = ptr; // copie du pointeur sur le d�but de la zone m�moire
  ve_size = size; // copie la taille
  i2c_address = address << 1; // copie l'adresse de ce p�riph�rique
...
void i2c_isr(void) interrupt 6 // 7 for C8051F330
{
  ...
  case I2C_SRADD:
    ...
    if (ACKRQ) {
      if (((SMB0DAT & 0xFE) == i2c_address) ||
          ((SMB0DAT & 0xFE) == I2C_BROADCAST)) // Write
      {
        STA = 0; // Chang� en sta 0
        cpt = 0;
        ACK = 1;
